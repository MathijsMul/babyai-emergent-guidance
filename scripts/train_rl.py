...
if acmodel is None:
    if args.pretrained_model:
        ...
        acmodel = ACModel(obs_space=obss_preprocessor.obs_space,
                             ...
                             arch=args.arch,
                             vocabulary=obss_preprocessor.vocab,
                             learner=args.learner,
                             corrector=args.corrector,
                             corr_length=args.corr_length,
                             corr_own_vocab=args.corr_own_vocab,
                             corr_vocab_size=args.corr_vocab_size,
                             pretrained_corrector=args.pretrained_corrector,
                             use_critic=True)
...
while status['num_frames'] < args.frames:
    ...
    if args.save_interval > 0 and status['i'] % args.save_interval == 0:
        ...
        agent.model = acmodel
        agent.model.eval()
        logs = batch_evaluate(agent, test_env_name, args.val_seed, args.val_episodes, args.show_corrections)
        agent.model.train()
