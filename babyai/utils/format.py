...
class Vocabulary:
    def __init__(self, model_name, max_vocab_size=30):
        self.path = get_vocab_path(model_name)
        self.max_size = max_vocab_size + 2 # 2 extra for <S> and ''

        if os.path.exists(self.path):
            self.vocab = json.load(open(self.path))
            self.idx2word = {self.vocab[key] : key for key in self.vocab.keys()}
        else:
            self.vocab = {}
            self.idx2word = {}
    ...
    def copy_vocab_from(self, other):
        ...
        self.idx2word.update(other.idx2word)

class InstructionsPreprocessor(object):
    def __init__(self, model_name, load_vocab_from=None, max_vocab_size=100):
        self.model_name = model_name
        self.vocab = Vocabulary(model_name, max_vocab_size)
        ...
        if not os.path.exists(path) and load_vocab_from is not None:
            ...
            if os.path.exists(secondary_path):
                old_vocab = Vocabulary(load_vocab_from, max_vocab_size)
                self.vocab.copy_vocab_from(old_vocab)
            else:
                raise FileNotFoundError('No vocabulary because no pre-trained model under the specified name')
...
class ObssPreprocessor:
    def __init__(self, model_name, obs_space, load_vocab_from=None, max_vocab_size=100):
        self.image_preproc = RawImagePreprocessor()
        self.instr_preproc = InstructionsPreprocessor(model_name, load_vocab_from, max_vocab_size)
