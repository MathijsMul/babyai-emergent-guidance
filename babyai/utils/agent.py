...
class ModelAgent(Agent):
    ...
    def __init__(self, model_or_name, obss_preprocessor, argmax):
        ...
        self.entropy = None
        self.corrector_script = False

    def set_corrector_script(self, script):
        self.corrector_script = True
        self.model.corrector.set_script(script=script)

    def act_batch(self, many_obs, show_corrections=False, time=None):
        ...
        if self.memory_corrector is None:
            self.memory_corrector = torch.zeros(
                len(many_obs), self.model.memory_size, device=self.device)
        if self.entropy is None:
            self.entropy = torch.zeros(len(many_obs), 1, device=self.device)

        preprocessed_obs = self.obss_preprocessor(many_obs, device=self.device)

        with torch.no_grad():
            model_results = self.model(obs=preprocessed_obs, memory=self.memory, memory_corrector=self.memory_corrector, previous_entropy=self.entropy, time=time)

            if show_corrections:
                print(model_results['corrections'])
            ...
            try:
                corr = model_results['corrections']
            except:
                corr = None
            self.memory = model_results['memory']
            self.memory_corrector = model_results['memory_corrector']
            self.entropy = model_results['dist'].entropy().unsqueeze(1)
    ...
    def act(self, obs, time=None):
        return self.act_batch([obs], time=time)
...
def load_agent(env, model_name, demos_name=None, demos_origin=None, argmax=True, env_name=None):
    ...
    if model_name == 'BOT':
        ...
        obss_preprocessor = utils.ObssPreprocessor(model_name, env.observation_space, load_vocab_from=model_name)
