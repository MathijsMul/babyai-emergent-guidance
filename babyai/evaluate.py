...
# Returns the performance of the agent on the environment for a particular number of episodes.
def batch_evaluate(agent, env_name, seed, episodes, seed_shift=1e9, return_obss_actions=False, show_corrections=False):
    ...
    for i in range((episodes + num_envs - 1) // num_envs):
        ...
        while (num_frames == 0).any():
            action = agent.act_batch(many_obs, show_corrections)['action']
